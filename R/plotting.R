...
myboxplot.formula=function(formula, data, cex=.5, xlab="", ylab=NULL, main="", box=TRUE, at=NULL, na.action=NULL, 
                           p.val=NULL, pch=1, col="white", col.points=1, border=1,  test="", friedman.test.formula=NULL, 
    ...
    if (length(test)>0) {
        ...
        if ("w" %in% test) {
            ...
            if (write.p.at.top) {sub=paste0("p=",signif(p.val,2))
            } else if (simplified) {sub="P: "%.%formatDouble(p.val,3)}
            else sub=sub%.%" Wilcoxon "%.%ifelse(length(test)==1,"p-val ","")%.%signif(p.val,2)
        }
        if ("k" %in% test) {
...
corplot.formula=function(formula,data,main="",method=c("pearson","spearman"),col=1,cex=.5,add.diagonal.line=TRUE,add.lm.fit=FALSE,add.loess.fit=FALSE,col.lm=2,add.deming.fit=FALSE,col.deming=4,add=FALSE, digit.cor=2,
    ...
    if (length(method)>0) {
        ...
        if (len(method)==1) {
          pval = formatPvalues(cor.test(data[,vars[1]],data[,vars[2]], method=method, use="pairwise.complete.obs")$p.value)
        } else pval = NULL
