...
# modified to include Poisson for independent working correlations
# Youyi Fong (Aug 31, 2025)
...
ssase = function(fit.gee) {
  ...
  if (fit.gee$family$family=="gaussian") {
    stopifnot(fit.gee$corstr %in% c("independence","exchangeable","ar1")) 
    stopifnot(fit.gee$family$link=="identity")
    
  } else if (fit.gee$family$family=="poisson") {
    stopifnot(fit.gee$corstr %in% c("independence")) 
    stopifnot(fit.gee$family$link=="log")
  }
  
  # the standard sandwich estimate uses alpha, gamma, A and B
  alpha = fit.gee$geese$alpha
  gamma = fit.gee$geese$gamma # dispersion
  ...
  sig = lapply(uid, function(id) {
    if (fit.gee$family$family=="gaussian") {
      ...
      w = which(ids==id)
      return (diag(1/mu[w], nc=clusz[id]))
    }
  })
